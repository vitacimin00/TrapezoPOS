...
private fun OpenShiftDialog(user: UserEntity, onDismiss: () -> Unit, onResult: (String) -> Unit) {
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text("Buka shift") }, text = { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) { Text("Kasir: ${user.name}"); OutlinedTextField(opening, { opening = it }, label = { Text("Modal awal") }, singleLine = true); error?.let { Text(it, color = MaterialTheme.colorScheme.error) } } }, confirmButton = { Button(onClick = { scope.launch { val cash = Money.parseOrNull(opening, allowBlank = true); if (cash == null) { error = "Modal awal tidak valid" } else when (val r = AppGraph.shifts.open(user, cash)) { is ShiftRepository.Result.Ok -> onResult("Shift berhasil dibuka"); is ShiftRepository.Result.Error -> error = r.message } } }) { Text("BUKA SHIFT") } }, dismissButton = { TextButton(onClick = onDismiss) { Text("BATAL") } })
}
...
private fun CashMovementDialog(shift: ShiftEntity, type: String, user: UserEntity, onDismiss: () -> Unit, onResult: (String) -> Unit) {
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text(label) }, text = { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) { OutlinedTextField(amount, { amount = it }, label = { Text("Nominal") }, singleLine = true); OutlinedTextField(note, { note = it }, label = { Text("Catatan") }); error?.let { Text(it, color = MaterialTheme.colorScheme.error) } } }, confirmButton = { Button(onClick = { scope.launch { val cash = Money.parseOrNull(amount); if (cash == null || cash <= 0) { error = "Nominal tidak valid" } else when (val r = AppGraph.shifts.cash(shift, type, cash, note, user.id)) { is ShiftRepository.Result.Ok -> onResult("$label tersimpan"); is ShiftRepository.Result.Error -> error = r.message } } }) { Text("SIMPAN") } }, dismissButton = { TextButton(onClick = onDismiss) { Text("BATAL") } })
}
...
private fun DiscountDialog(current: OrderDiscount, subtotal: Long, onDismiss: () -> Unit, onApply: (OrderDiscount) -> Unit) {
    var kind by remember { mutableStateOf(current.kind) }; var value by remember { mutableStateOf(if (current.kind == DiscountKind.NONE) "" else current.value.toString()) }; var error by remember { mutableStateOf<String?>(null) }
    val maxDiscount = if (kind == DiscountKind.PERCENT) 100L else Money.MAX_RUPIAH
    val parsedValue = Money.parseOrNull(value, max = maxDiscount, allowBlank = kind == DiscountKind.NONE)
    AlertDialog(onDismissRequest = onDismiss, title = { Text("Diskon transaksi") }, text = { Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
        listOf(DiscountKind.NONE to "Tanpa diskon", DiscountKind.NOMINAL to "Nominal (Rp)", DiscountKind.PERCENT to "Persentase (%)").forEach { (id,label) -> Row(verticalAlignment = Alignment.CenterVertically) { RadioButton(kind == id, { kind = id; error = null }); Text(label) } }
        if (kind != DiscountKind.NONE) OutlinedTextField(value, { value = it; error = null }, label = { Text(if (kind == DiscountKind.PERCENT) "Persen" else "Nominal") }, singleLine = true)
        if (value.isNotBlank() && parsedValue == null) Text("Nilai diskon tidak valid", color = MaterialTheme.colorScheme.error)
        val proposed = OrderDiscount(kind, parsedValue ?: 0L).amountFor(subtotal); Text("Diskon: ${Money.fmt(proposed)}")
        error?.let { Text(it, color = MaterialTheme.colorScheme.error) }
    } }, confirmButton = { Button(onClick = { if (kind == DiscountKind.NONE) onApply(OrderDiscount(kind, 0L)) else { val v = parsedValue; if (v == null) error = "Nilai diskon tidak valid" else onApply(OrderDiscount(kind, v)) } }) { Text("TERAPKAN") } }, dismissButton = { TextButton(onClick = onDismiss) { Text("BATAL") } })
}
...
private fun PaymentDialog(total: Long, methods: List<Pair<String, String>>, onDismiss: () -> Unit, onComplete: (Map<String, Long>, Map<String, String>) -> Unit) {
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text("Pembayaran • ${Money.fmt(total)}") }, text = { Column(Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        OutlinedButton(onClick = { val n = Money.parseOrNull(amount); if (methodId.isBlank()) error = "Aktifkan metode pembayaran terlebih dahulu" else if (n == null || n <= 0) error = "Nominal tidak valid" else { val existing = tenders[methodId] ?: 0L; val sum = Money.addExact(existing, n); if (sum == null) { error = "Nominal pembayaran melampaui batas" } else { val copy = LinkedHashMap(tenders); copy[methodId] = sum; val candidate = PaymentAllocation.settle(copy, total); if (methodId != PaymentAllocation.CASH && candidate.settled.isEmpty()) { error = "Pembayaran non-tunai tidak boleh melebihi sisa tagihan" } else { tenders = copy; val refs = LinkedHashMap(references); if(reference.isNotBlank()) refs[methodId] = reference; references = refs; amount = (total - candidate.settled.values.sum()).coerceAtLeast(0).toString(); reference = ""; error = null } } } }, Modifier.fillMaxWidth(), enabled = methods.isNotEmpty()) { Text("TAMBAH PEMBAYARAN") }
