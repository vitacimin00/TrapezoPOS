...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** Adds persistent login throttling without touching existing passwords/accounts. */
        ...
        private fun backfillRefundPayments(db: SupportSQLiteDatabase) {
            ...
            fun originalMethods(saleId: Long): LinkedHashMap<String, Long> = originalCache.getOrPut(saleId) {
                var grandTotal = 0L
                db.query("SELECT grandTotal FROM sales WHERE id=? LIMIT 1", arrayOf(saleId)).use { cursor ->
                    if (cursor.moveToFirst()) grandTotal = cursor.getLong(0).coerceAtLeast(0)
                }
                ...
                db.query(
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        val amount = cursor.getLong(1).coerceAtLeast(0)
        ...
        /**
         * First-run seed intentionally creates NO user account. TrapezoRoot opens
         * the local owner-setup screen and UserRepository.bootstrapAdmin creates
         * the first administrator atomically with a user-chosen password.
         */
